          ...
          <div className="relative">
             ...
             {showTasks && (
                 <>
                    ...
                    <div className="absolute right-0 top-full mt-2 w-[calc(100vw-2rem)] md:w-80 bg-white rounded-xl shadow-xl border border-gray-200 z-20 overflow-hidden animate-in fade-in slide-in-from-top-2 max-h-[80vh] flex flex-col">
                            ...
                            {tasks.length === 0 ? (
                                ...
                                tasks.map(task => (
                                    <div key={task.id} className="p-3 border-b border-gray-100 hover:bg-gray-50 flex gap-3 relative group">
                                        ...
                                        <div 
                                            className={`flex-1 min-w-0 ${task.status === 'COMPLETED' ? 'cursor-pointer' : ''}`}
                                            onClick={() => handleTaskClick(task)}
                                        >
                                            <p className="text-sm font-medium text-gray-800 truncate">{task.meta.title}</p>
